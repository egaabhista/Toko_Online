...
if (mysqli_num_rows($result_keranjang) > 0) {
    ...
    try {
        ...
        if ($id_petugas) {
            $insert_transaksi = mysqli_query($koneksi, "INSERT INTO toko_transaksi (id_pelanggan, id_petugas, tgl_transaksi) VALUES ('$id_pelanggan', '$id_petugas', '$tgl_transaksi')");
        } else {
            $insert_transaksi = mysqli_query($koneksi, "INSERT INTO toko_transaksi (id_pelanggan, tgl_transaksi) VALUES ('$id_pelanggan', '$tgl_transaksi')");
        }
        ...
        $id_transaksi = mysqli_insert_id($koneksi); // Ambil ID transaksi terakhir yang disisipkan
        ...
        while ($row = mysqli_fetch_assoc($result_keranjang)) {
            ...
            $insert_detail = mysqli_query($koneksi, "INSERT INTO toko_detail_transaksi (id_transaksi, id_produk, qty, subtotal) VALUES ('$id_transaksi', '".$row['id_produk']."', '$qty', '$subtotal')");
        ...
        mysqli_query($koneksi, "DELETE FROM toko_keranjang WHERE id_pelanggan = '$id_pelanggan'");
